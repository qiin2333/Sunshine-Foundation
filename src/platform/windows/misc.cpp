...
namespace platf {
  ...
  launch_sunshine_in_user_session() {
    // Only works when running as SYSTEM (service mode)
    if (!is_running_as_system()) {
      BOOST_LOG(warning) << "launch_sunshine_in_user_session() called but not running as SYSTEM"sv;
      return false;
    }

    // Get user token for the active console session
    HANDLE user_token = retrieve_users_token(false);
    if (!user_token) {
      BOOST_LOG(error) << "Failed to retrieve user token for launching Sunshine in user session"sv;
      return false;
    }
    ...
    // Get Sunshine executable path
    WCHAR executable[MAX_PATH];
    if (GetModuleFileNameW(NULL, executable, ARRAYSIZE(executable)) == 0) {
      auto winerr = GetLastError();
      BOOST_LOG(error) << "Failed to get Sunshine path: "sv << winerr;
      return false;
    }
    ...
    // Launch Sunshine in user session
    BOOL success = FALSE;
    std::error_code ec = impersonate_current_user(user_token, [&]() {
      success = CreateProcessAsUserW(user_token,
        executable,
    ...
    if (ec || !success) {
      auto winerr = GetLastError();
      BOOST_LOG(error) << "Failed to launch Sunshine in user session: "sv << winerr;
      return false;
    }

    BOOST_LOG(info) << "Successfully launched Sunshine in user session (PID: "sv << process_info.dwProcessId << ')' << std::endl;
    BOOST_LOG(info) << "Service mode instance will exit. Please use the user session instance instead."sv;
